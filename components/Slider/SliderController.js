...
const SliderController = ({ scrollToNext , scrollToPrev , color , bgColor , position }) => {
    ...
    return (
        <>
            <TouchableOpacity style={[style.item , { left : "0%" , backgroundColor : bgColor || "transparent"}]} onPress={scrollToPrev}>
                <Feather name="arrow-left" size={24} color={color || "black"} />
            </TouchableOpacity>
            <TouchableOpacity style={[style.item , { right : "0%" , backgroundColor : bgColor || "transparent" }]} onPress={scrollToNext}>
